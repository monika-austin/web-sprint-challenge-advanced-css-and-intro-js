...
const correctedArtists = [...artists];
correctedArtists[8].name = "Vincent Van Gogh";
console.log(correctedArtists);
...
function get20s(array) {
  ...
  for (i = 0; i < clonedArray.length; i++) {
    if (
      (!clonedArray[i].years.includes("18") &&
        clonedArray[i].years.includes("19")) ||
      clonedArray[i].years.includes("2000")
    ) {
...
function lotsOfArt(array) {
  ...
  let artistsName = [];
  for (let i = 0; i < lotsOfArt.length; i++) {
    if (lotsOfArt[i].paintings > 100) {
      artistsName.push(lotsOfArt[i].name);
    }
  }
  return artistsName;
}
